...
int i;
int result_code;
cpu_set_t cpusetp, cpusetp2, cpuset_zero, cpuset_sec_ext, cpuset_k, cpuset_stream0, cpuset_stream1, cpuset_stream2, cpuset_stream3, cpuset_stream4, cpuset_stream5, cpuset_stream6, cpuset_stream7, cpuset_stream8;
...
struct thread_args_stream *args_stream7=NULL ;
struct thread_args_stream *args_stream8=NULL ;
...
struct thread_args_sample *args_sample7=NULL ;
struct thread_args_sample *args_sample8=NULL ;
...
__global__ void GPU_compute_k_split_enc(unsigned short *d_mpr, unsigned int * d_len, int * d_code){
    ...
    __shared__ unsigned short mpr[64];
    unsigned int len[1];
    unsigned short code[1];
...
void *compute_sample0(void * input){
    ...
    bitStream_store_constant(args->compressed_stream, args->written_bytes, args->written_bits, 1, 0);
    // Register size
    bitStream_store(args->compressed_stream, args->written_bytes, args->written_bits, 6, 64);
    // Weight resolution
    bitStream_store(args->compressed_stream, args->written_bytes, args->written_bits, 4, 15);
    // weight update scaling exponent change interval
    bitStream_store(args->compressed_stream, args->written_bytes, args->written_bits, 4, ((unsigned int)log2((float)64)) - 4);
    // weight update scaling exponent initial parameter
    bitStream_store(args->compressed_stream, args->written_bytes, args->written_bits, 4, 5);
    // weight update scaling exponent final parameter
    bitStream_store(args->compressed_stream, args->written_bytes, args->written_bits, 4, 9);
    // reserved
    bitStream_store_constant(args->compressed_stream, args->written_bytes, args->written_bits, 1, 0);
    // weight initialization method and weight initialization table flag
    ...
    /* ENTROPY CODER METADATA */
        // Unary length limit
        bitStream_store(args->compressed_stream, args->written_bytes, args->written_bits, 5, 18);
        // rescaling counter size
        bitStream_store(args->compressed_stream, args->written_bytes, args->written_bits, 3, 2);
        // initial count exponent
        bitStream_store(args->compressed_stream, args->written_bytes, args->written_bits, 3, 1);
        // Accumulator initialization constant and table
        bitStream_store(args->compressed_stream, args->written_bytes, args->written_bits, 4, 3);
        bitStream_store_constant(args->compressed_stream, args->written_bytes, args->written_bits, 1, 0);
...
void *compute_sample(void *input){
    ...
    for(z = 0; z < args->bands; z++){
        args->counter[z] = 0x1 << 1;
        args->accumulator[z] = (args->counter[z]*(3*(0x1 << (3 + 6))-49))/0x080;
            ...
            for(x = 0; x < args->input_params.x_size; x++){
                ...
                if((y == 0 && x == 0)){
                    ...
                    if(divisor < 18){
                        ...
                        bitStream_store_constant(args->compressed_stream, args->written_bytes, args->written_bits, 18, 0);
                        //if(args->written_bytes[0]==198752/8 || args->written_bytes[0]==198744/8)
                    ...
                    // ... and finally update the statistics
                    if(args->counter[z] < ((((unsigned int)0x1) << 6) -1)){
...
void *compute_zero_code(void * input){
    ...
    printf("compute_zero_code=%lf\n",(end5.tv_sec-start5.tv_sec)*1e3+(end5.tv_nsec-start5.tv_nsec)*1e-6);
    FILE *fp;
    fp = fopen("tempos_compute_zero_code.txt","a");
    if(fp == NULL){
        printf("Error writng file!\n");   
        exit(1);             
    }
...
void *compute_sec_ext(void *input){
    ...
    printf("compute_sec_ext=%lf\n",(end4.tv_sec-start4.tv_sec)*1e3+(end4.tv_nsec-start4.tv_nsec)*1e-6);
    FILE *fp;
    fp = fopen("tempos_compute_sec_ext.txt","a");
    if(fp == NULL){
        printf("Error writng file!\n");   
        exit(1);             
    }
...
void *compute_k(void * input){
    ...
    FILE *fp;
    fp = fopen("tempos_compute_k.txt","a");
    if(fp == NULL){
        printf("Error writng file!\n");   
        exit(1);             
    }
...
void *compute_stream_arm(void * input){
    ...
    //printf("const_zero=%u, const_less=%u, const_diff_zero=%u, zero=%u, less=%u, diff_zero=%u, for1=%u, for2=%u, big_zero=%u\n", const_zero, const_less, const_diff_zero, zero, less, diff_zero, for1, for2, big_zero);
    clock_gettime(CLOCK_MONOTONIC, &end4);
    printf("ARM %d=%lf\n",args->id, (end4.tv_sec-start4.tv_sec)*1e3+(end4.tv_nsec-start4.tv_nsec)*1e-6);
    FILE *fp;
    fp = fopen("tempos_arm.txt","a");
    if(fp == NULL){
        printf("Error writng file!\n");   
...
void *compute_stream0(void * input){
    ...
    if(args->written_bits[0]==0){
        for(i=0; i<args->written_bytes7[0];i++){
            args->compressed_stream[args->written_bytes[0]]=args->stream7[i];
            args->written_bytes[0]++;
        }
    }
    else{
        for(i=0; i<=args->written_bytes7[0];i++){
            args->compressed_stream[args->written_bytes[0]]|=(args->stream7[i]>>args->written_bits[0]);
            args->written_bytes[0]++;
            args->compressed_stream[args->written_bytes[0]]=(args->stream7[i]<<(64-args->written_bits[0]));
        }
        if(args->written_bits[0]+args->written_bits7[0] <64){
            args->compressed_stream[args->written_bytes[0]]|=(args->stream7[i]>>args->written_bits[0]);
            args->written_bytes[0]--;
        }
        /* else{
            args->compressed_stream[args->written_bytes[0]]|=(args->stream7[i]>>args->written_bits[0]);
            args->written_bytes[0]++;
            args->compressed_stream[args->written_bytes[0]]=(args->stream7[i]<<(args->written_bits7[0]-(args->written_bits7[0]+args->written_bits[0])%64));

        } */
    }

    args->written_bits[0]=(args->written_bits[0]+args->written_bits7[0])%64;
    //printf("byte=%u, bit=%u, stream_byte=%u, stream_bit=%u\n", args->written_bytes[0],args->written_bits[0],args->written_bytes6[0], args->written_bits6[0]);



    if(args->written_bits[0]==0){
        for(i=0; i<=args->written_bytes8[0];i++){
            args->compressed_stream[args->written_bytes[0]]=args->stream8[i];
            args->written_bytes[0]++;
        }
    }
    else{
        for(i=0; i<=args->written_bytes8[0];i++){
            args->compressed_stream[args->written_bytes[0]]|=(args->stream8[i]>>args->written_bits[0]);
            args->written_bytes[0]++;
            args->compressed_stream[args->written_bytes[0]]=(args->stream8[i]<<(64-args->written_bits[0]));
        }
        if(args->written_bits[0]+args->written_bits8[0] <64){
            args->compressed_stream[args->written_bytes[0]]|=(args->stream8[i]>>args->written_bits[0]);
            args->written_bytes[0]--;
        }

    }
    args->written_bits[0]=(args->written_bits[0]+args->written_bits8[0])%64; 
 ...
 */
    ...
    unsigned char * stream_zero_codes7=NULL;
    unsigned int * stream_len_sec_ext7=NULL;
    unsigned int * stream_len_k7=NULL;
    int * stream_k_codes7=NULL;
    unsigned int * stream_sec_ext_codes7=NULL;
    unsigned long * stream7=NULL;
    unsigned int * written_bytes7=NULL;
    unsigned int * written_bits7=NULL;

    unsigned char * stream_zero_codes8=NULL;
    unsigned int * stream_len_sec_ext8=NULL;
    unsigned int * stream_len_k8=NULL;
    int * stream_k_codes8=NULL;
    unsigned int * stream_sec_ext_codes8=NULL;
    unsigned long * stream8=NULL;
    unsigned int * written_bytes8=NULL;
    unsigned int * written_bits8=NULL;
    ...
    stream_zero_codes7=(unsigned char *)malloc(sizeof(unsigned char));
    if(stream_zero_codes7 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    stream_len_sec_ext7=(unsigned int *)malloc(sizeof(unsigned int));
    if(stream_len_sec_ext7 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    stream_len_k7=(unsigned int *)malloc(sizeof(unsigned int));
    if(stream_len_k7 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    stream_k_codes7=( int *)malloc(sizeof( int));
    if(stream_k_codes7 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    stream_sec_ext_codes7=(unsigned int *)malloc(sizeof(unsigned int));
    if(stream_sec_ext_codes7 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    stream7=(unsigned long *)malloc((((input_params.dyn_range + 7)/8)*input_params.x_size*input_params.y_size*input_params.z_size));
    if(stream7 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }
    memset(stream7, 0, (((input_params.dyn_range + 7)/8)*input_params.x_size*input_params.y_size*input_params.z_size));

    written_bytes7=(unsigned int *)malloc(sizeof(unsigned int));
    if(written_bytes7 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    written_bits7=(unsigned int *)malloc(sizeof(unsigned int));
    if(written_bits7 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }
    ...
    //==============================================================================================================================
    //stream 8
    stream_zero_codes8=(unsigned char *)malloc(sizeof(unsigned char));
    if(stream_zero_codes8 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    stream_len_sec_ext8=(unsigned int *)malloc(sizeof(unsigned int));
    if(stream_len_sec_ext8 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    stream_len_k8=(unsigned int *)malloc(sizeof(unsigned int));
    if(stream_len_k8 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    stream_k_codes8=( int *)malloc(sizeof( int));
    if(stream_k_codes8 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    stream_sec_ext_codes8=(unsigned int *)malloc(sizeof(unsigned int));
    if(stream_sec_ext_codes8 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    stream8=(unsigned long *)malloc((((input_params.dyn_range + 7)/8)*input_params.x_size*input_params.y_size*input_params.z_size));
    if(stream8 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }
    memset(stream8, 0, (((input_params.dyn_range + 7)/8)*input_params.x_size*input_params.y_size*input_params.z_size));

    written_bytes8=(unsigned int *)malloc(sizeof(unsigned int));
    if(written_bytes8 == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }

    written_bits8=(unsigned int *)malloc(sizeof(unsigned int));
    if(written_bits == NULL){
        printf("Failed to allocate host mpr\n" );
        exit(EXIT_FAILURE);
    }
    ...
    //evita que a memoria seja usada para swap
    #ifdef CRISM //2779500
        unsigned int arm=347438;
        //unsigned int arm=((input_params.x_size*input_params.y_size*input_params.z_size)/64)/8;
    #endif
    
    #ifdef CASI
        unsigned int arm=69940;
    #endif
     ...
     /* #ifdef CRISM //2779500
        unsigned int arm=((input_params.x_size*input_params.y_size*input_params.z_size)/64)/4;
    #endif
    
    #ifdef CASI
        unsigned int arm=279760;
    #endif
    ...
    args_stream7 = (struct thread_args_stream *)malloc(sizeof(struct thread_args_stream));
    args_stream8 = (struct thread_args_stream *)malloc(sizeof(struct thread_args_stream));
    ...
    args_stream0->stream7=&stream7[0];
    args_stream0->written_bytes7=&written_bytes7[0];
    args_stream0->written_bits7=&written_bits7[0];
    args_stream0->stream8=&stream8[0];
    args_stream0->written_bytes8=&written_bytes8[0];
    args_stream0->written_bits8=&written_bits8[0];
    ...
    args_stream1->n_blocks=arm;
    args_stream1->id=1;


    args_stream2->ptr_stream=&residuals[arm*64];
    args_stream2->stream_zero_codes=&zero_codes[arm];
    args_stream2->stream_len_sec_ext=&len_sec_ext[arm];
    args_stream2->stream_len_k=&len_k[arm];
    args_stream2->stream_k_codes=&k_codes[arm];
    args_stream2->stream_sec_ext_codes=&sec_ext_codes[(arm*64)/2];
    ...
    args_stream2->n_blocks=arm;
    args_stream2->id=2;

    args_stream3->ptr_stream=&residuals[(arm*2)*64];
    args_stream3->stream_zero_codes=&zero_codes[(arm*2)];
    args_stream3->stream_len_sec_ext=&len_sec_ext[(arm*2)];
    args_stream3->stream_len_k=&len_k[(arm*2)];
    args_stream3->stream_k_codes=&k_codes[(arm*2)];
    args_stream3->stream_sec_ext_codes=&sec_ext_codes[((arm*2)*64)/2];
    ...
    args_stream4->ptr_stream=&residuals[(arm*3)*64];
    args_stream4->stream_zero_codes=&zero_codes[(arm*3)];
    args_stream4->stream_len_sec_ext=&len_sec_ext[(arm*3)];
    args_stream4->stream_len_k=&len_k[(arm*3)];
    args_stream4->stream_k_codes=&k_codes[(arm*3)];
    args_stream4->stream_sec_ext_codes=&sec_ext_codes[((arm*3)*64)/2];
    ...
    args_stream5->ptr_stream=&residuals[(arm*4)*64];
    args_stream5->stream_zero_codes=&zero_codes[(arm*4)];
    args_stream5->stream_len_sec_ext=&len_sec_ext[(arm*4)];
    args_stream5->stream_len_k=&len_k[(arm*4)];
    args_stream5->stream_k_codes=&k_codes[(arm*4)];
    args_stream5->stream_sec_ext_codes=&sec_ext_codes[((arm*4)*64)/2];
    ...
    args_stream6->ptr_stream=&residuals[(arm*5)*64];
    args_stream6->stream_zero_codes=&zero_codes[(arm*5)];
    args_stream6->stream_len_sec_ext=&len_sec_ext[(arm*5)];
    args_stream6->stream_len_k=&len_k[(arm*5)];
    args_stream6->stream_k_codes=&k_codes[(arm*5)];
    args_stream6->stream_sec_ext_codes=&sec_ext_codes[((arm*5)*64)/2];
    ...
    args_stream7->ptr_stream=&residuals[(arm*6)*64];
    args_stream7->stream_zero_codes=&zero_codes[(arm*6)];
    args_stream7->stream_len_sec_ext=&len_sec_ext[(arm*6)];
    args_stream7->stream_len_k=&len_k[(arm*6)];
    args_stream7->stream_k_codes=&k_codes[(arm*6)];
    args_stream7->stream_sec_ext_codes=&sec_ext_codes[((arm*6)*64)/2];
    args_stream7->compressed_stream=&stream7[0];
    args_stream7->written_bytes=&written_bytes7[0];
    args_stream7->written_bits=&written_bits7[0];
    args_stream7->id=7;
    args_stream7->n_blocks=arm;
    


    args_stream8->ptr_stream=&residuals[(arm*7)*64];
    args_stream8->stream_zero_codes=&zero_codes[(arm*7)];
    args_stream8->stream_len_sec_ext=&len_sec_ext[(arm*7)];
    args_stream8->stream_len_k=&len_k[(arm*7)];
    args_stream8->stream_k_codes=&k_codes[(arm*7)];
    args_stream8->stream_sec_ext_codes=&sec_ext_codes[((arm*7)*64)/2];
    args_stream8->compressed_stream=&stream8[0];
    args_stream8->written_bytes=&written_bytes8[0];
    args_stream8->written_bits=&written_bits8[0];
    args_stream8->id=8;
    args_stream8->n_blocks=arm;
    ...
    CPU_ZERO(&cpuset_stream7);
    CPU_SET(6, &cpuset_stream7);
    
    CPU_ZERO(&cpuset_stream8);
    CPU_SET(7, &cpuset_stream8); 
    ...
    if(pthread_attr_init(&(threads_attr[12])) != 0)
        printf("pthread_attr_init");
    if(pthread_attr_setaffinity_np(&(threads_attr[12]), sizeof(cpu_set_t), &cpuset_stream7) != 0)
        printf("pthread_attr_setaffinity_np");

    if(pthread_attr_init(&(threads_attr[13])) != 0)
        printf("pthread_attr_init");
    if(pthread_attr_setaffinity_np(&(threads_attr[13]), sizeof(cpu_set_t), &cpuset_stream8) != 0)
        printf("pthread_attr_setaffinity_np");
    ...
    pthread_create(&threads[6], &(threads_attr[6]), compute_stream_arm, args_stream1);
    pthread_create(&threads[7], &(threads_attr[7]), compute_stream_arm, args_stream2);
    ...
    pthread_create(&threads[10], &(threads_attr[12]), compute_stream_arm, args_stream7);
    pthread_create(&threads[11], &(threads_attr[13]), compute_stream_arm, args_stream8);
 ...
 */
    ...
    if(stream_zero_codes1 != NULL)
        free(stream_zero_codes1);
    if(stream_len_sec_ext1 != NULL)
        free(stream_len_sec_ext1);
    if(stream_len_k1 != NULL)
        free(stream_len_k1);
    if(stream_k_codes1 != NULL)
        free(stream_k_codes1);
    if(stream_sec_ext_codes1 != NULL)
        free(stream_sec_ext_codes1);
    if(stream1 != NULL)
        free(stream1);
    if(written_bytes1 != NULL)
        free(written_bytes1);
    ...
    if(stream_zero_codes2 != NULL)
        free(stream_zero_codes2);
    if(stream_len_sec_ext2 != NULL)
        free(stream_len_sec_ext2);
    if(stream_len_k2 != NULL)
        free(stream_len_k2);
    if(stream_k_codes2 != NULL)
        free(stream_k_codes2);
    if(stream_sec_ext_codes2 != NULL)
        free(stream_sec_ext_codes2);
    if(stream2 != NULL)
        free(stream2);
    if(written_bytes2 != NULL)
        free(written_bytes2);
    ...
    if(stream_zero_codes3 != NULL)
        free(stream_zero_codes3);
    if(stream_len_sec_ext3 != NULL)
        free(stream_len_sec_ext3);
    if(stream_len_k3 != NULL)
        free(stream_len_k3);
    if(stream_k_codes3 != NULL)
        free(stream_k_codes3);
    if(stream_sec_ext_codes3 != NULL)
        free(stream_sec_ext_codes3);
    if(stream3 != NULL)
        free(stream3);
    if(written_bytes3 != NULL)
        free(written_bytes3);
    ...
    if(stream_zero_codes4 != NULL)
        free(stream_zero_codes4);
    if(stream_len_sec_ext4 != NULL)
        free(stream_len_sec_ext4);
    if(stream_len_k4 != NULL)
        free(stream_len_k4);
    if(stream_k_codes4 != NULL)
        free(stream_k_codes4);
    if(stream_sec_ext_codes4 != NULL)
        free(stream_sec_ext_codes4);
    if(stream4 != NULL)
        free(stream4);
    if(written_bytes4 != NULL)
        free(written_bytes4);
    ...
    if(stream_zero_codes5 != NULL)
        free(stream_zero_codes5);
    if(stream_len_sec_ext5 != NULL)
        free(stream_len_sec_ext5);
    if(stream_len_k5 != NULL)
        free(stream_len_k5);
    if(stream_k_codes5 != NULL)
        free(stream_k_codes5);
    if(stream_sec_ext_codes5 != NULL)
        free(stream_sec_ext_codes5);
    if(stream5 != NULL)
        free(stream5);
    if(written_bytes5 != NULL)
        free(written_bytes5);
    ...
    if(stream_zero_codes6 != NULL)
        free(stream_zero_codes6);
    if(stream_len_sec_ext6 != NULL)
        free(stream_len_sec_ext6);
    if(stream_len_k6 != NULL)
        free(stream_len_k6);
    if(stream_k_codes6 != NULL)
        free(stream_k_codes6);
    if(stream_sec_ext_codes6 != NULL)
        free(stream_sec_ext_codes6);
    if(stream6 != NULL)
        free(stream6);
    if(written_bytes6 != NULL)
        free(written_bytes6);
    ...
    if(stream_zero_codes7 != NULL)
        free(stream_zero_codes7);
    if(stream_len_sec_ext7 != NULL)
        free(stream_len_sec_ext7);
    if(stream_len_k7 != NULL)
        free(stream_len_k7);
    if(stream_k_codes7 != NULL)
        free(stream_k_codes7);
    if(stream_sec_ext_codes7 != NULL)
        free(stream_sec_ext_codes7);
    if(stream7 != NULL)
        free(stream7);
    if(written_bytes7 != NULL)
        free(written_bytes7);
    ...
    if(stream_zero_codes8 != NULL)
        free(stream_zero_codes8);
    if(stream_len_sec_ext8 != NULL)
        free(stream_len_sec_ext8);
    if(stream_len_k8 != NULL)
        free(stream_len_k8);
    if(stream_k_codes8 != NULL)
        free(stream_k_codes8);
    if(stream_sec_ext_codes8 != NULL)
        free(stream_sec_ext_codes8);
    if(stream8 != NULL)
        free(stream8);
    if(written_bytes8 != NULL)
        free(written_bytes8);
    if(written_bits8 != NULL)
        free(written_bits8);
    ...
    if(args_stream2 != NULL)
        free(args_stream2);
    if(args_stream3 != NULL)
        free(args_stream3);
    if(args_stream4 != NULL)
        free(args_stream4);
    if(args_stream5 != NULL)
        free(args_stream5);
    if(args_stream6 != NULL)
        free(args_stream6);
    if(args_stream7 != NULL)
        free(args_stream7);
    if(args_stream8 != NULL)
        free(args_stream8);
