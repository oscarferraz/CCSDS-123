...
unsigned int const_zero=0;
    unsigned int const_less=0;
    unsigned int const_diff_zero=0;
    unsigned int zero=0;
    unsigned int less=0;
    unsigned int diff_zero=0;
    unsigned int for1=0;
    unsigned int for2=0;
    unsigned int big_zero=0;
...
void bitStream_store(unsigned long * compressed_stream, unsigned int * written_bytes,
                    unsigned int * written_bits, unsigned int num_bits_to_write, unsigned long bits_to_write){
    //if(*written_bytes==67)
    ...
    if(bits_to_write==0){
        *written_bytes+=(*written_bits + num_bits_to_write)/64;
        *written_bits= (*written_bits +num_bits_to_write)%64;
        //zero++;
    }
    else{
        if(num_bits_to_write + (*written_bits) <64){
            compressed_stream[*written_bytes] |= (bits_to_write << (64-(*written_bits)- num_bits_to_write));
            (*written_bits)+=num_bits_to_write;
            //less++;
        }
        else{
            if(*written_bits !=0){
                compressed_stream[*written_bytes] |= bits_to_write >> (num_bits_to_write -(64-(*written_bits)));
                num_bits_to_write-= 64 -(*written_bits);
                *written_bits =0;
                (*written_bytes)++;
                //diff_zero++;
            }
            /* for(int i=num_bits_to_write /32; i>0;i--){
                if(num_bits_to_write%32==0){
                    compressed_stream[*written_bytes] |= (bits_to_write >> (i -1) * 32) & 0xff;
                    //for1++;
                }
                else{
                    compressed_stream[*written_bytes] |= (bits_to_write >> (num_bits_to_write-((i)*32)))& 0xff;
            ...
            if(num_bits_to_write >0){
                bits_to_write=bits_to_write&((0x1 << num_bits_to_write)-1);
                compressed_stream[*written_bytes] |= (bits_to_write << (64 -num_bits_to_write));
                (*written_bits)+=num_bits_to_write;
                //big_zero++;
            }
        }
...
void bitStream_store_constant(unsigned long * compressed_stream, unsigned int * written_bytes,
    ...
    if(bit_to_repeat==0){
        ...
        *written_bits= (*written_bits +num_bits_to_write)%64;
        ...
        bit_to_repeat = 0x1;
        if(num_bits_to_write + (*written_bits)<64){
            compressed_stream[*written_bytes] |= ((long)((0x2 << (num_bits_to_write -1))-1) << (64-(*written_bits)-num_bits_to_write));
            *written_bits+= num_bits_to_write;
            //const_less++;
        }
        else{
            if(*written_bits!=0){
                compressed_stream[*written_bytes] |= (0x2 << (64-(*written_bits))-1)-1;
                num_bits_to_write -=64-(*written_bits);
                *written_bits=0;
                (*written_bytes)++;
                //const_diff_zero++;
            ...
            for(y = 0; y < input_params.y_size; y++){
                for(x = 0; x < input_params.x_size; x++){
                    if(input_params.signed_samples != 0){
                        ...
                        aux=sample_buffer & 0xFF00;
                        aux=aux >> 8;
                        sample_buffer=sample_buffer << 8;
                        sample_buffer=sample_buffer+aux;
