...
//check result
#if defined (VERIFICATION)  || defined (TEX_VERIFICATION)
    extern unsigned int *h_local_sum;
    extern signed int *h_scaled_predicted;
    extern signed int *h_delta;
    extern unsigned short *h_omega;
    extern unsigned short *h_mpr;
#endif
...
int check_omega=0;
int check_mpr=0;
struct timespec start, end, soma;
...
int local_sum(input_feature_t input_params, predictor_config_t predictor_params,
    ...
    //soma.tv_sec+=end.tv_sec-start.tv_sec;
    //soma.tv_nsec+=end.tv_nsec-start.tv_nsec;
...
int compute_local_differences(input_feature_t input_params, predictor_config_t predictor_params,
    ...
    if(((*local_differences)[0] = (int *)malloc(sizeof(int)*input_params.x_size*input_params.y_size*input_params.z_size)) == NULL){
        fprintf(stderr, "Error in allocating %lu bytes for holding the local differences matrix\n", sizeof(int)*input_params.x_size*input_params.y_size*input_params.z_size);
        return -1;
    }
    if(predictor_params.full != 0){
        int i = 0;
        for(i = 1; i < 4; i++){
            if(((*local_differences)[i] = (int *)malloc(sizeof(int)*input_params.x_size*input_params.y_size*input_params.z_size)) == NULL){
                fprintf(stderr, "Error in allocating %lu bytes for holding the local differences matrix %d\n", sizeof(int)*input_params.x_size*input_params.y_size*input_params.z_size, i);
                return -1;
            }
        }
    }
...
#endif  
                //printf("x=%d, y=%d, z=%d, value=%u\n", x, y, z, h_cld[x+y*input_params.x_size+(z*input_params.y_size*input_params.x_size)]);
                //check local sum
                /*#if defined (VERIFICATION)  || defined (TEX_VERIFICATION)
                    if(check_local==0)
                        printf("\n\nVERIFING THE RESULTS OF THE LOCAL SUM DIFFERENCE\n");
                    if(h_local_sum[x+y*input_params.x_size+(z*input_params.y_size*input_params.x_size)]!=local_sum_temp){
                        printf("local sum different at x=%d, y=%d, z=%d\n", x, y, z);
                        return -1;
                    }
                    check_local=1;
                #endif*/
                MATRIX_BSQ_INDEX((*local_differences)[0], input_params, x, y, z) = 4*MATRIX_BSQ_INDEX(samples, input_params, x, y, z) - local_sum_temp;
                //check central local difference
                /*#if defined (VERIFICATION)  || defined (TEX_VERIFICATION)
                    if(check==0)
                        printf("VERIFING THE RESULTS OF THE CENTRAL LOCAL DIFFERENCE\n");
                    if(h_cld[x+y*input_params.x_size+(z*input_params.y_size*input_params.x_size)]!=MATRIX_BSQ_INDEX((*local_differences)[0], input_params, x, y, z)){
                        printf("central local sum different at x=%d, y=%d, z=%d\n", x, y, z);
                        return -1;
                    }
...
//     fprintf(stderr, "compute_predicted_sample=%d, %d, %d - pred_bands = %d\n", x, y, z, predictor_params.pred_bands);
    ...
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // local sum + scaled predict value
    //clock_gettime(CLOCK_MONOTONIC, &start);
    if(x > 0 || y > 0){
...
//             fprintf(stderr, "sample\n");
    ...
    /*#if defined (VERIFICATION)  || defined (TEX_VERIFICATION)
            ...
            if(check_mod==0)
                printf("VERIFING THE RESULTS OF THE MOD STAR\n");
            if(h_scaled_predicted[x+y*input_params.x_size+(z*input_params.y_size*input_params.x_size)]!=scaled_predicted){
                printf("mod_star different at x=%d, y=%d, z=%d, serial=%lli, cuda=%u\n", x, y, z,scaled_predicted,
                 h_scaled_predicted[x+y*input_params.x_size+(z*input_params.y_size*input_params.x_size)] );
                exit(EXIT_FAILURE);
            }
...
unsigned short int compute_mapped_residual(input_feature_t input_params, 
    ...
    /*#if defined (VERIFICATION)  || defined (TEX_VERIFICATION)
        ...
        if(check_delta==0)
        printf("VERIFING THE RESULTS OF DELTA\n");
        if(h_delta[x+y*input_params.x_size+(z*input_params.y_size*input_params.x_size)]!=delta){
            printf("delta different at x=%d, y=%d, z=%d, serial=%d, cuda=%d\n", x, y, z,delta,
                h_delta[x+y*input_params.x_size+(z*input_params.y_size*input_params.x_size)] );
            exit(EXIT_FAILURE);
        }
        check_delta=1;
    #endif*/
    ...
    if(omega > s_max - scaled_predicted/2){
     ...
     /* #if defined (VERIFICATION)  || defined (TEX_VERIFICATION)
        if(check_omega==0)
        printf("VERIFING THE RESULTS OF OMEGA\n");
        if(h_omega[x+y*input_params.x_size+(z*input_params.y_size*input_params.x_size)]!=omega){
            printf("omega different at x=%d, y=%d, z=%d, serial=%hu, cuda=%d\n", x, y, z,omega,
                h_omega[x+y*input_params.x_size+(z*input_params.y_size*input_params.x_size)] );
            exit(EXIT_FAILURE);
        }
        check_omega=1;
    #endif */
    ...
    /* #if defined (VERIFICATION)  || defined (TEX_VERIFICATION)
        if(check_mpr==0)
        printf("VERIFING THE RESULTS OF MAPPED RESIDUAL\n");
        if(h_mpr[x+y*input_params.x_size+(z*input_params.y_size*input_params.x_size)]!=mapped){
            printf("mpr different at x=%d, y=%d, z=%d, serial=%hu, cuda=%d\n", x, y, z,mapped,
                h_mpr[x+y*input_params.x_size+(z*input_params.y_size*input_params.x_size)] );
            exit(EXIT_FAILURE);
        }
        check_mpr=1;
    #endif */
...
int predict(input_feature_t input_params, predictor_config_t predictor_params, char inputFile[128], unsigned short int * residuals){
    ...
    // CUDA -------------------------------------------------------------------------------------
    unsigned short int * cuda_samples = NULL;    
    unsigned short * local_sum=NULL;
    unsigned int * central_local_difference=NULL;
    ...
    //CUDA------------------------------------------------------------------------------------------------------
    // Parse the input image, loading it into memory and appropriately converting it
    /*cuda_samples=samples_cuda_malloc(input_params);
    if(cuda_samples == NULL){
        fprintf(stderr, "Error in allocating %lf kBytes for the input image buffer\n\n", ((double)sizeof(unsigned short int)*input_params.x_size*input_params.y_size*input_params.z_size)/1024.0);
        return -1;
    }
    if(read_samples(input_params, inputFile, cuda_samples) != 0){
        return -1;
    }*/

    //cuda_compute_prediction(input_params, predictor_params,samples);
    //central_local_difference=central_local_difference_cuda(input_params, predictor_params, samples);
...
//                 fprintf(stderr, "sample=%d\n", MATRIX_BSQ_INDEX(samples, input_params, x, y, z));
   ...
   fprintf(fp,"%lf\n",(soma.tv_sec*1e3)+(soma.tv_nsec*1e-6));
   fclose(fp);*/
