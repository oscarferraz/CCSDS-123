...
void *compute_stream_arm(void * input){
    ...
    if(args->id == -1){
        printf("DENVER core=%d, BITSTREAM=%lf\n",args->n_blocks, (end4.tv_sec-start4.tv_sec)*1e3+(end4.tv_nsec-start4.tv_nsec)*1e-6); 
        ...
        printf("ARM core=%d, BITSTREAM=%lf\n",args->n_blocks, (end4.tv_sec-start4.tv_sec)*1e3+(end4.tv_nsec-start4.tv_nsec)*1e-6); 
...
void *compute_stream0(void * input){
    ...
    args->written_bits[0]=(args->written_bits[0]+args->written_bits1[0])%64;
    //printf("byte=%u, bit=%u, stream_byte=%u, stream_bit=%u\n", args->written_bytes[0],args->written_bits[0],args->written_bytes2[0+1], args->written_bits2[0]);
 ...
 */
    ...
    #ifdef CASI
    //559 519
        unsigned int denver=122000;
        unsigned int arm=78880;
    #endif

    #ifdef HAWAII
        unsigned int denver=240000;
        unsigned int arm=155072;
    #endif

    #ifdef YELLOWSTONE
        unsigned int denver=266000;
        unsigned int arm=171640;
    #endif

    #ifdef CRISM
        unsigned int denver=605000;
        unsigned int arm=392375;
    #endif

    /* #ifdef CASI
    //559 519
        unsigned int arm=139880;
    #endif

    #ifdef HAWAII
        unsigned int arm=275072;
    #endif

    #ifdef YELLOWSTONE
        unsigned int arm=304640;
    #endif

    #ifdef CRISM
        unsigned int arm=694875;
    #endif */
    ...
    CPU_ZERO(&cpuset_stream2);
    CPU_SET(3, &cpuset_stream2);


    CPU_ZERO(&cpuset_stream3);
    CPU_SET(4, &cpuset_stream3);

    CPU_ZERO(&cpuset_stream4);
    CPU_SET(5, &cpuset_stream4);
