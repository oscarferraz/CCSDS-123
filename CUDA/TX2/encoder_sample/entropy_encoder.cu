...
//printf("%llu\n", millisecondsSinceEpoch);
    ...
    #ifdef CRISM
        unsigned int arm=88;
        unsigned int denver=96;
    #endif

    #ifdef CASI
        unsigned int arm=15;
        unsigned int denver=6;
    #endif 
    ...
    /* counter1 = (unsigned int *)malloc(sizeof(unsigned int)*denver);
    if(counter1 == NULL){
        fprintf(stderr, "Error in the allocation of the counter2 statistic\n\n");
        return -1;
    }
    accumulator1 = (unsigned int *)malloc(sizeof(unsigned int)*denver);
    if(accumulator1 == NULL){
        fprintf(stderr, "Error in the allocation of the accumulator1 statistic\n\n");
        return -1;
    }

    counter2 = (unsigned int *)malloc(sizeof(unsigned int)*denver);
    if(counter2 == NULL){
        fprintf(stderr, "Error in the allocation of the counter2 statistic\n\n");
        return -1;
    }
    accumulator2 = (unsigned int *)malloc(sizeof(unsigned int)*denver);
    ...
    #ifdef CRISM
        counter6 = (unsigned int *)malloc(sizeof(unsigned int)*(arm+1));
        if(counter6 == NULL){
            fprintf(stderr, "Error in the allocation of the counter6 statistic\n\n");
            return -1;
        }
        accumulator6 = (unsigned int *)malloc(sizeof(unsigned int)*(arm+1));
    ...
    args_sample2->residuals=&residuals[denver*input_params.y_size*input_params.x_size];
    args_sample3->residuals=&residuals[(2*denver)*input_params.y_size*input_params.x_size];
    args_sample4->residuals=&residuals[((2*denver)+arm)*input_params.y_size*input_params.x_size];
    ...
    args_sample1->bands=denver;
    args_sample2->bands=denver;
    ...
    CPU_SET(4, &cpuset_stream0);
    CPU_SET(5, &cpuset_stream0);
