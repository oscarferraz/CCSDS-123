...
#include <getopt.h>
#else
#include "/home/nvidia/Desktop/ccsds123_oscar/CUDA/encoder_oscar/win32_getopt.h"
#endif
...
int main(int argc, char *argv[]){
    ...
    struct timespec compressionEndTime, compressionStartTime, predictionEndTime;
    ...
    do{
        ...
        switch(foundOpt){
            ...
            break;
            case 27:
                predictor_params.weight_initial = (signed char)atoi(optarg);
            break;
            case 28:
                predictor_params.weight_final = (signed char)atoi(optarg);
            break;
            case 29:
    ...
    if(predictor_params.pred_bands < 0 || predictor_params.pred_bands > 15 ){
        fprintf(stderr, "\nError, please specify the prediction bands between 0 and 15\n\n");
        fprintf(stderr, USAGE_STRING, argv[0]);
        return -1;
    }
    ...
    if(predictor_params.register_size < 32){
        fprintf(stderr, "\nError, the register size cannot be smaller than 32\n\n");
        fprintf(stderr, USAGE_STRING, argv[0]);
        return -1;
    }
    if(predictor_params.register_size < 2+predictor_params.weight_resolution+input_params.dyn_range){
        fprintf(stderr, "\nError, the register size cannot be smaller than 2+D+\u03A9\n\n");
        fprintf(stderr, USAGE_STRING, argv[0]);
        return -1;
    }
    ...
    if(predictor_params.weight_initial < -6 || predictor_params.weight_initial > predictor_params.weight_final){
        fprintf(stderr, "\nError, the weight initial value must be in the range [-6, Vmax]\n\n");
        fprintf(stderr, USAGE_STRING, argv[0]);
        return -1;
    }
    if(predictor_params.weight_final > 9 || predictor_params.weight_final < predictor_params.weight_initial){
        fprintf(stderr, "\nError, the weight final value must be in the range [vmin, 9]\n\n");
        fprintf(stderr, USAGE_STRING, argv[0]);
        return -1;
    }
    ...
    clock_gettime(CLOCK_MONOTONIC, &compressionStartTime);
    ...
    clock_gettime(CLOCK_MONOTONIC, &predictionEndTime);
    ...
    clock_gettime(CLOCK_MONOTONIC, &compressionEndTime);
    ...
    printf("Overall Compression duration %lf (msec)\n",(compressionEndTime.tv_sec-compressionStartTime.tv_sec)*1e3+(compressionEndTime.tv_nsec-compressionStartTime.tv_nsec)*1e-6);
    printf("Prediction duration %lf (msec)\n", (predictionEndTime.tv_sec-compressionStartTime.tv_sec)*1e3+(predictionEndTime.tv_nsec-compressionStartTime.tv_nsec)*1e-6);
    printf("Encoding duration %lf (msec)\n", (compressionEndTime.tv_sec-predictionEndTime.tv_sec)*1e3+(compressionEndTime.tv_nsec-predictionEndTime.tv_nsec)*1e-6 );
