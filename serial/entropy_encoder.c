    ...
    if((y == 0 && x == 0)){
        ...
        if(divisor < encoder_params.u_max){
         ...
         //if(x==142 &&y==4 && z==3)
                   ...
                   // printf("x=%d, y=%d, counter=%u, accumulator=%u, temp=%d, div=%u, rem=%u, mpr=%hu, bits=%hu, value=%hhu, result=%d\n", x, y, counter[z], accumulator[z],temp_k, divisor, reminder, residuals[curIndex], written_bits[0],compressed_stream[written_bytes[0]], (int)log2(((49*counter[z])/0x080 + accumulator[z])/((double)counter[z])));
...
int encode_sampleadaptive(input_feature_t input_params, encoder_config_t encoder_params, unsigned short int * residuals,
    ...
    printf("BITSTREAM encoder=%lf\n",(end3.tv_sec-start3.tv_sec)*1e3+(end3.tv_nsec-start3.tv_nsec)*1e-6);

    FILE *fp;
    fp = fopen("tempos_encoder_inner.txt","a");
    if(fp == NULL){
