...
int main(int argc, char *argv[]){
    ...
    if(predictor_params.pred_bands < 0 || predictor_params.pred_bands > 16 ){
    ...
    printf("Overall Compression duration %lf (msec)\n", ((end1.tv_sec-start1.tv_sec)*1e3+(end1.tv_nsec-start1.tv_nsec)*1e-6)+((end2.tv_sec-start2.tv_sec)*1e3+(end2.tv_nsec-start2.tv_nsec)*1e-6));
    
    FILE *fp_p, *fp_e;
    fp_p = fopen("tempos_preditor.txt","a");
    if(fp_p == NULL){
        printf("Error opening predictor file!\n");   
        exit(1);             
    }

    fp_e = fopen("tempos_encoder.txt","a");
    if(fp_e == NULL){
        printf("Error opening encoder file!\n");   
        exit(1);             
    }
